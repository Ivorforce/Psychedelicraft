...
//            double d7 = (double)(entityRenderer.thirdPersonDistanceTemp + (entityRenderer.thirdPersonDistance - entityRenderer.thirdPersonDistanceTemp) * partialTicks);
            ...
            for (int k = 0; k < 8; ++k)
            {
                ...
                f4 *= 0.1F;
                f5 *= 0.1F;
                MovingObjectPosition movingobjectposition = mc.theWorld.rayTraceBlocks(mc.theWorld.getWorldVec3Pool().getVecFromPool(d0 + (double) f3, d1 + (double) f4, d2 + (double) f5), mc.theWorld.getWorldVec3Pool().getVecFromPool(d0 - d3 + (double) f3 + (double) f5, d1 - d5 + (double) f4, d2 - d4 + (double) f5));

                if (movingobjectposition != null)
                {
                    double d6 = movingobjectposition.hitVec.distanceTo(mc.theWorld.getWorldVec3Pool().getVecFromPool(d0, d1, d2));
